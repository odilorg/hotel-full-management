                            ...
                            <form action="{{ route('inkassas.store') }}" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="custom-select rounded-0" name="report_id"
                                        ...
                                        <option
                                            {{ old('report_number', $inkassa->report_id) == $number->report_number ? "selected" : "" }}
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group " id="reservationdate" data-target-input="nearest">
                                        <input type="text" value="{{ old('date_inkassa', $inkassa->date_inkassa) }}" name="date_inkassa" class="form-control date @error('date_inkassa')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{{ old('amount_inkassa', $inkassa->amount_inkassa) }}" name="amount_inkassa" class="form-control  @error('amount_inkassa')
                 {{ 'is-invalid' }} @enderror " id="inputError" placeholder="Inkassa Amount">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="custom-select rounded-0" name="firm_id"
                                        id="exampleSelectRounded0">
                                        @foreach ($firm_names as $firm )
                                        <option
                                            {{ old('firm_id', $inkassa->firm_id) == $firm->id ? "selected" : "" }}
                                            value="{{ $firm->id }}">{{ $firm->firm_name }}</option>
                                        @endforeach
                                    </select>
                                    @error('firm_id')
                                    <p class="text-danger">{{ $message }}</p>
